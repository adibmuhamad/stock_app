...
class ChatBotUtil {
  ...
  ChatBotUtil({required String sessionId}) : _sessionId = sessionId;
  ...
  Future<ResponseData> ask(String question) async {
    if (_snlm0e == null) {
      await _initialize();
    }
    ...
    List<String> choices = [];
    jsonChatData[2].map((i) => {choices.add(i[0])});

    ResponseData chatResponse = ResponseData(
      ask: jsonChatData[2][0][0],
      answer: jsonChatData[4][0][1][0],
    );
    ...
    final results = {
      ...
      "choices": jsonChatData[2]
          .map((i) => {"id": i[1].toString(), "content": i[0]})
          .toList(),
    };
